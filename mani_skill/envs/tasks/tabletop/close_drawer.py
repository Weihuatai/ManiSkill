...
class CloseDrawerEnv(BaseEnv):
    """
    **Task Description:**
    - Use the Panda manipulation robot on the ground to close the target drawer.

    **Randomizations:**(No randomization for position of the cabinet)
    ...
    **Success Conditions:**
    - The drawer is close at least 5% of the way, and the angular/linear velocities of the drawer link are small

    **Goal Specification:**
    - 3D goal position centered at the center of mass of the handle mesh on the drawer to close (also visualized in human renders with a sphere).
    """
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.all_model_ids = np.array([
            model_id for model_id in train_data.keys()
            if model_id not in excluded_ids
        ])
    ...
    def _default_human_render_camera_configs(self):
        pose = sapien_utils.look_at(eye=[-0.6, -1, 1], target=[0.5, 0.5, 0])
        return CameraConfig(
    ...
    def _after_reconfigure(self, options):
        ...
        # Target position changed to near closed state
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            qmin = qlimits[..., 0]                         # closed
            qmax = qlimits[..., 1]                         # fully open
            init_qpos = qmin.clone()
            joint_idx = self.handle_link.joint.index.to(self.device)  # [b]
            # clamp to valid DOF range
            joint_idx = torch.clamp(joint_idx, 0, init_qpos.size(1) - 1)
            # select per-env qmax for the target joint
            idx = torch.arange(b, device=self.device)
            max_vals = qmax[idx, joint_idx]  # shape [b]
            init_qpos[idx, joint_idx] = max_vals

            self.cabinet.set_qpos(init_qpos)
    ...
    def evaluate(self):
        ...
        closed_enough = self.handle_link.joint.qpos <= self.target_qpos  # Negate the judgment condition
    ...
    def compute_dense_reward(self, obs: Any, action: torch.Tensor, info: Dict):
        ...
        reaching_reward = 1 - torch.tanh(5 * tcp_to_handle_dist)
         # how much closing remains (clamped 0–1)
        current = self.handle_link.joint.qpos
        remain = torch.clamp((current - self.target_qpos) / (current + 1e-6), 0.0, 1.0)
        close_reward = 2 * (1 - remain)
        # once any closing starts, focus on closing reward
        reaching_reward[remain < 0.999] = 2
        # max bonus when fully closed enough
