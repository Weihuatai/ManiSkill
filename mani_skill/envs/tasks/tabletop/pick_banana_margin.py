...
@register_env("PickBananaMargin-v1", max_episode_steps=150, asset_download_ids=["ycb"])
class PickBananaMarginEnv(BaseEnv):
    """
    **Task Description:**
    - Pick up a banana from the [YCB dataset](https://www.ycbbenchmarks.com/) and move it to a random goal position

    **Randomizations:**
    - the banana's xy position is randomized on top of a table in the region X ∈ [-0.5, 0], Y ∈ ([-0.5, -0.3] or [0.3, 0.5]) to avoid the camera view.
    - the banana is placed flat on the table
    - the goal position X, Y ∈ [-0.1, 0.1], Z ∈ [object height, object height + 0.3]
    ...
    **Success Conditions:**
    ...
    def _load_scene(self, options: dict):
        ...
        # load a single banana model across all parallel environments
        ...
        # assign the banana actor to all env instances
        builder.set_scene_idxs(list(range(self.num_envs)))
        banana = builder.build(name=self.model_id)
        self.remove_from_state_dict_registry(banana)
        self._objs.append(banana)
    ...
    def _initialize_episode(self, env_idx: torch.Tensor, options: dict):
        with torch.device(self.device):
            ...
            # change position to avoid camera view
            xyz[:, 0] = torch.rand((b,)) * 0.5 - 0.5
            # define the y position to be in two ranges
            mask = torch.rand(b, device=self.device) < 0.5
            # left section [0.3,0.5]
            high = torch.rand(b, device=self.device) * 0.2 + 0.3
            # right section [-0.5,-0.3]
            low  = torch.rand(b, device=self.device) * 0.2 - 0.5
            # assign y position based on the mask
            xyz[:, 1] = torch.where(mask, high, low)
            ...
            # sample the goal position close to the object
            # goal_offset_xy = torch.rand((b, 2), device=self.device) * 0.2 - 0.1
            # goal_xyz = xyz.clone()
            # goal_xyz[:, :2] += goal_offset_xy
            # goal_xyz[:, 2] = torch.rand((b)) * 0.3 + xyz[:, 2]
